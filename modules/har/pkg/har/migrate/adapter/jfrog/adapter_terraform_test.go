...
func makeTerraformTestFiles(uris ...string) []types.File {
	files := make([]types.File, len(uris))
	for i, uri := range uris {
		parts := splitTerraformTestPath(uri)
...
func splitTerraformTestPath(uri string) []string {
	var parts []string
	cur := ""
	for _, c := range uri {
		if c == '/' {
